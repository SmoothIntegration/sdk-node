...
describe('QuickBooks: getConsentUrl', () => {
    test('Returns Consent Url on 200 Response', async () => {
        mockFetch({
            request: {
                method: 'GET',
                url: 'https://api.smooth-integration.com/quickbooks/connect?company_id=a5c8d02c-f8dd-45ee-9495-53c1781501b7',
            },
            response: {
                status: 200,
                body: JSON.stringify({
    ...
    test('Returns Consent Url for SandBox companies on 200 Response', async () => {
        mockFetch({
            request: {
                method: 'GET',
                url: 'https://api.smooth-integration.com/quickbooks/connect?company_id=a5c8d02c-f8dd-45ee-9495-53c1781501b7&sandbox=true',
            },
            response: {
                status: 200,
                body: JSON.stringify({
    ...
    test('Raises SIError: Unauthorized on 401 Response', async () => {
        mockFetch({
            request: {
                method: 'GET',
                url: 'https://api.smooth-integration.com/quickbooks/connect?company_id=invalid-company-id',
            },
            response: {
                status: 401,
