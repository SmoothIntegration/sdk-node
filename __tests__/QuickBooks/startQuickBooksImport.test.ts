...
describe('QuickBooks: startImport', () => {
    test('Returns void on 200 Response', async () => {
        ...
        await client.quickBooks.startImport(TEST_DATASOURCE_ID);
    });
    ...
    test('Raises SIError: Bad Request on 400 Response', async () => {
        ...
        const client = new SIClient(TEST_CLIENT_ID, TEST_CLIENT_SECRET);
        await expect(client.quickBooks.startImport(TEST_DATASOURCE_ID)).rejects.toThrow(
